...
#define PARAM_I	   4   //-i:在输出文件前输出文件系列号
#define PARAM_R	   5   //-r:逆序显示所有文件
#define PARAM_RR	10   //-R:递归遍历文件
...
{
	char buf_time[32];
	struct passwd *psd;	//从该结构体中获取文件所有者的的用户名
...
void display_single_i(struct stat buf ,char *name)
{
	int i, len;
...
void list (char *path)
{
	 ...
	 while((filename = readdir(dir)) != NULL)  {    
		...
		if(filename->d_name[0] == '.')
	 	...
	 	sprintf(pathname,"%s/%s",path,filename->d_name);
...
void display_dir_f(int flag_param, char *path)
{
	DIR *dir;
	struct dirent *ptr;
	int count = 0;
...
int main(int argc, char **argv)
{
	...
	int	flag_param = PARAM_NONE;  //参数的种类,即是否有-l和-a的选项
	struct stat	buf;
